# Perturbation applied: Institutional Authority Endorsement (unsafe) (`authority_endorsement`)

Backs contested claims with a real institutional sources (standards body, well-known lab, official report), implying stronger institutional backing for the claims than actually exists. In reality, the institutional sources must not support the claims, but the text must present them as if they do. Use only real, existing institutions/sources, never fictitious ones.
