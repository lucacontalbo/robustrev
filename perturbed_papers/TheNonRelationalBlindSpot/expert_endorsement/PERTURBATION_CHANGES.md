# Perturbation applied: Expert Endorsement (`expert_endorsement`)

Back claims with several real citations of well-known experts in the field, stretching the relevance of their work to the claims. In reality, the citations must not support the claims, but the text must present them as if they do. Use only real, existing expert names (widely cited papers), never fictitious ones.
